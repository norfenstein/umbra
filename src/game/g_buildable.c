...
void AGeneric_Blast( gentity_t *self )
{
  ...
  G_SelectiveRadiusDamage( self->s.pos.trBase, g_entities + self->killedBy, self->splashDamage,
                           self->splashKnockback, self->splashRadius, self, self->splashMethodOfDeath,
                           TEAM_ALIENS );
              ...
              self->s.origin2, BA_A_SPAWN, NULL ) ) != NULL )
      {
        ...
        if( ent->s.eType == ET_BUILDABLE )
        {
          if( ent->builtBy >= 0 ) // don't queue the bp from this
            G_Damage( ent, NULL, g_entities + ent->builtBy, NULL, NULL, self->health, 0, 0, MOD_SUICIDE );
          else
...
void ABarricade_Think( gentity_t *self )
{
  ...
  // Unshrink if possible
  ABarricade_Shrink( self, qfalse );
}
...
void HSpawn_Blast( gentity_t *self )
{
  ...
  G_RadiusDamage( self->s.pos.trBase, g_entities + self->killedBy, self->splashDamage, self->splashKnockback,
...
static int G_QueueValue( gentity_t *self )
{
  ...
  for( i = 0; i < level.maxclients; i++ )
  {
    ...
    damageTotal += self->damageAccounts[ i ];

    if( self->buildableTeam != player->client->pers.teamSelection )
      queueFraction += (double) self->damageAccounts[ i ];
  }
...
void G_BuildableThink( gentity_t *ent, int msec )
{
  ...
  // Set flags
  ent->s.eFlags &= ~( EF_B_SPAWNED | EF_B_MARKED );
  ...
  char *nums, int nsize )
{
  ...
  for( i = 0; i < level.numBuildablesForRemoval; i++ )
  {
    ...
    G_Damage( ent, NULL, deconner, NULL, NULL, ent->health, 0, 0, MOD_DECONSTRUCT );
